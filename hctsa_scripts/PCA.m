...
for D = 1:length(Subs)   
   ...
   % Index for each conventional stage (W,N1,N2,N3,R) -> markers
    ...
    for C = 1:5
        for S = 1:5 
        Table{C,S} = intersect(Clusters{C},Stages{S});
        end
    end
    ...
    f = figure; ax=gca;
     ...
     for S = 1:5  
        for C = 1:5    % 5 clusters x 5 stages, potentially 25 combinations of markers / colors
            ...
            hold on
            scatter(mappedX(Table{C,S},1),mappedX(Table{C,S},2),100,Colors{C},Markers{S},'LineWidth',1.3)
...
% figure; scatter(Centroid(1), Centroid(2),1000,'Marker', '*','MarkerEdgeColor','k','LineWidth',50) 
