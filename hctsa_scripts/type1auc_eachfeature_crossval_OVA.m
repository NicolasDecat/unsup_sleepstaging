...
% Get epochs indices for each "rest of stage"
wake_rest = [N1 N2 N3 rem];
N1_rest = [wake N2 N3 rem];
N2_rest = [wake N1 N3 rem];
N3_rest = [wake N1 N2 rem];
rem_rest = [wake N1 N2 N3];

% All binary classifiers
allpairs = [{wake} {N1} {N2} {N3} {rem};{wake_rest} {N1_rest} {N2_rest} {N3_rest} {rem_rest}];

for Nit = 1:size(testMat,2)
    
    testMatr = testMat(:,Nit);  % One iteration of testMat at a time

    for C = 1:5         % For each binary classifier
        
        % Choose one classifier
        classifier1 = allpairs{1,C};   
        classifier2 = allpairs{2,C}; 
        ...
        for test = 1:stgL           % each epoch will be taken out and used as testing epoch
            ...
            test_stage2 = stage2(randi(numel(stage2)));            
            
            % Training data (N-1 epochs for each stage)
            stage1 = setdiff(stage1,test_stage1);   % Leave one epoch out ("test" epoch)
            stage2 = setdiff(stage2,test_stage2);    % Take randomly 1 epoch from the 43 elements
...
%             IsStage2_stored{C} = IsStage2;
         ...
         Per_correct_stage1(Nit,C) = (length(find(ismember(IsStage1,classifier1)))/stgL)*100;   % Percentage of epochs from stage 1 labeled right
         Per_correct_stage2(Nit,C) = (length(find(ismember(IsStage2,classifier2)))/stgL)*100;   % Percentage of epochs from stage 1 labeled right 
