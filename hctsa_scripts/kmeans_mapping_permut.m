...
function [statsOut testMat scoredTest predictTest Nf Iteration NumChannels Dataset Sleep_stage Testing_accuracy AUC] = cross_validation_selectivefeatures_permut(experiment, hctsa_ops, cm_save_dir, number_of_channels_used, epochSelectFunction, selective_feature,sub,v,col)
...
%% Test set are classified into 5 classes(clusters) obtained from
    ...
    predictTesttoPerm = stats.predictTest(Nf,:);
    predictTestPerm(Nf,:) = predictTesttoPerm(randperm(length(predictTesttoPerm)));
    
    
end % End Nf-th randomisation
% 
% x = 10;
% length(find(predictTesttoPerm == 0))
% length(find(predictTesttoPerm == 1))
% length(find(predictTesttoPerm == 2))
% length(find(predictTesttoPerm == 3))
% length(find(predictTesttoPerm == 5))
% 
% length(find(predictTestPerm(x,:) == 0))
% length(find(predictTestPerm(x,:) == 1))
% length(find(predictTestPerm(x,:) == 2))
% length(find(predictTestPerm(x,:) == 3))
% length(find(predictTestPerm(x,:) == 5))
