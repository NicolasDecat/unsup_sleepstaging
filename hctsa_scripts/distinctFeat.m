...
for D = 1:length(Subs)
    ...
    datamat = datam;
    datamat = datamat.TS_DataMat;   % Take feature values from 1 feature

    feat_id = 1:size(datamat,2);          % How many features (1)
   ...
   PVAL_CW_iter = mean(PVAL_CW);
   PVAL_C1_iter = mean(PVAL_C1);
   PVAL_C2_iter = mean(PVAL_C2);
   PVAL_C3_iter = mean(PVAL_C3);
   PVAL_CR_iter = mean(PVAL_CR);
   
   % Re arrange into one array (Row 1 = C1, Row 5 = CR, Col = all F)
   PVAL_ALL_iter = [PVAL_CW_iter;PVAL_C1_iter;PVAL_C2_iter;PVAL_C3_iter;PVAL_CR_iter];
